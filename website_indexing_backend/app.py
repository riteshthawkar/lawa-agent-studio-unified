...
app = FastAPI(
    title="Website Indexing API",
    description="Concurrent website scraping, processing, and vector indexing pipeline",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc"
)
...
async def send_progress_webhook(
    ...
    This is a lightweight notification (no retries) to avoid slowing down processing.
    ...
    try:
        ...
        # Send webhook without retries (progress updates are best-effort)
        async with aiohttp.ClientSession() as session:
            try:
                async with session.post(
                    ...
                    data=body,
                    headers=headers,
                    timeout=aiohttp.ClientTimeout(total=10)  # Shorter timeout for progress
                ) as response:
                ...
                logger.debug("Progress webhook timeout (non-critical)")
                return False
            except Exception as e:
                logger.debug(f"Progress webhook error (non-critical): {e}")
                return False
...
async def process_indexing_task(
    ...
    """
    Background task to process website indexing.
    This runs concurrently and doesn't block other requests.
    Sends progress webhooks at key stages for real-time updates.
    """

    try:
        # Claim task and set status to processing
        # This resolves the race condition/deadlock where Django holds the lock
        # while waiting for this API to return. We do the DB update here in background.
        ...
        if not claimed:
            logger.error(f"❌ Failed to claim task {task_id} (ext: {external_job_id}). Aborting background processing.")
            return
        ...
        # Initialize processor inside background task to verify response speed
        # NOTE: This initialization might block the event loop if heavy.
        # Ideally should run in thread pool if CPU bound.
        ...
        if callback_url:
            ...
            # Remove raw CrawlResult objects which are not JSON serializable
...
async def start_indexing(
    ...
    if not site_id:
        ...
        # Use full URL for domain matching (Site model expects full URL)
...
async def delete_namespace(
    ...
    Used when a project (Site) is deleted.
    """
    # Bearer-token authentication for API security
    api_token = os.getenv("INDEXING_API_TOKEN")
    if api_token:
...
async def startup_event():
    ...
    try:
        ...
        # We bypass DocumentEmbedder entirely to avoid loading sentence-transformers
        ...
        if success:
            ...
            # We don't raise here to allow app to start, but functionality will be limited
