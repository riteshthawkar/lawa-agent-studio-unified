...
class DjangoDatabaseManager:
    ...
    def _sanitize_for_json(self, value: Any) -> Any:
        ...
        Removes non-serializable objects like CrawlResult from Crawl4AI.
        """
        if value is None:
            return None
        ...
        if isinstance(value, dict):
            return {k: self._sanitize_for_json(v) for k, v in value.items()
                    if k not in ('results', 'crawl_results')}  # Exclude non-serializable keys
        if isinstance(value, (list, tuple)):
            ...
            for item in value:
                try:
                    ...
                    continue
            return sanitized
        # Try to convert to a serializable representation
        try:
            import json
            json.dumps(value)
            return value
    ...
    async def update_task_status(self, task_id: str, status: str, **kwargs) -> bool:
        ...
        async def _update_job():
            ...
            for field, value in kwargs.items():
                if hasattr(IndexingJob, field):
                    ...
                    # Ensure error_message is never None (database doesn't allow NULL)
            ...
            # Use filter().update() for bulk update, but fetch first to update Site
            try:
        ...
        # Production Fix #6: Database resilience with retry
        max_retries = 3
        last_error = None
    ...
    async def get_excluded_patterns(self, site_id: str) -> List[Dict[str, Any]]:
        ...
        Returns:
            List of pattern dictionaries with pattern, pattern_type, and matches_url method
        """
        @sync_to_async
        def _get_patterns():
            try:
                # Import ExcludedURLPattern model
                ...
                patterns = ExcludedURLPattern.objects.filter(
                    site_id=site_uuid,
                    is_active=True
                )
