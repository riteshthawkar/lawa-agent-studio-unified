...
def token_a(api_session, base_url, user_a_data):
    ...
    # Verify User A
    _verify_user_via_shell(user_a_data['email'])
    ...
    if resp.status_code != 200:
        raise ValueError(f"Login failed for User A: {resp.text}")
...
def token_b(api_session, base_url, user_b_data):
    ...
    # Verify User B
    _verify_user_via_shell(user_b_data['email'])
    ...
    if resp.status_code != 200:
        raise ValueError(f"Login failed for User B: {resp.text}")

    return resp.json()['tokens']['access']

def _verify_user_via_shell(email):
    import subprocess
    import sys
    import os
    ...
    cmd = [sys.executable, "manage.py", "shell", "-c", verify_script]
    env = os.environ.copy()
