...
def create_site(request):
    ...
    try:
        ...
        if not org_id and request.user.is_authenticated:
            # Fallback to user's first organization if not provided in request context
            # This is common in tests where the middleware might not set org_id
            membership = Membership.objects.filter(user=request.user, organization__status='active').first()
            if membership:
