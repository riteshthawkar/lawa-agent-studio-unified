...
class TestMultiTenancyEnforcement(TestCase):
    ...
    def test_indexing_jobs_filters_by_organization(self):
        ...
        assert data['count'] == 1
        assert data['results'][0]['site_id'] == str(self.site1.id)
    ...
    def test_bulk_actions_respects_organization(self):
        ...
        # Only chatbot1 should be deactivated (if bulk actions is implemented to filter)
        self.chatbot1.refresh_from_db()
        self.chatbot2.refresh_from_db()
        # Note: Bulk actions logic might vary, but for multitenancy it MUST respect org boundaries
    ...
    def test_site_detail_cross_organization_access_denied(self):
        ...
        url = f'/v1/frontend/sites/{self.site2.id}/'
        response = self.client.get(url)

        # Should return 403 or 404 depending on implementation (403 is better for security)
    ...
    def test_search_respects_organization(self):
        ...
        self.client.force_authenticate(user=self.user1)
        url = reverse('sites-management')
        response = self.client.get(url, {'search': 'example1'})
        ...
        assert data['count'] == 1
        assert data['results'][0]['domain'] == 'https://example1.com'
