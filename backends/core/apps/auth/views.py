            ...
            with transaction.atomic():
                ...
                try:
                    ...
                    if "CERTIFICATE_VERIFY_FAILED" in str(e):
                        ...
                        # Reraise other exceptions to trigger transaction rollback
...
class SupportRequestViewSet(BaseViewSet):
    """
    ViewSet for Support Requests (Feedback)
    GET / — List
    POST / — Create
    DELETE /{id}/ — Delete
    """
...
class APIKeyViewSet(BaseViewSet):
    ...
    def get_queryset(self):
        ...
        if not org_id and 'HTTP_X_ORG_ID' in request.META:
             org_id = request.META['HTTP_X_ORG_ID']
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            serializer = self.get_serializer(data=request.data)
            serializer.is_valid(raise_exception=True)
            api_key = serializer.save(org_id=org_id)
            
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        except Exception as e:
            logger.error(f"Error creating API key: {str(e)}", exc_info=True)
            return Response({'error': f'Failed to create API key: {str(e)}'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def rotate(self, request, pk=None):
        ...
        try:
            ...
            # Store plaintext token temporarily for response
            new_key.token = token
