...
class LawaIntegration:
    ...
    async def get_chatbot_starters(self, api_key: str) -> Optional[list]:
        """
        Get conversation starters for a chatbot by API key.
        
        Args:
            api_key: The chatbot API key
            
        Returns:
            List of starter objects (id, question) or None if chatbot invalid
        """
        if not self.enabled or not self.pool:
            return None
        ...
        try:
            async with self.pool.acquire() as conn:
                # First get chatbot ID
                chatbot_query = "SELECT id FROM chatbots WHERE api_key = $1 AND status = 'active'"
                chatbot_id = await conn.fetchval(chatbot_query, api_key)
            ...
            logger.error(f"Error getting chatbot starters: {e}")
            return []
