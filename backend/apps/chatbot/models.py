...
class Chatbot(BaseModel):
    ...
    def get_widget_url(self):
        """Get widget JavaScript file URL"""
        from django.conf import settings
        widget_host = getattr(settings, 'WIDGET_CDN_HOST', 'localhost')
        widget_port = getattr(settings, 'WIDGET_CDN_PORT', '5174')
        
        # Determine protocol for widget
        protocol = 'https' if widget_host != 'localhost' else 'http'
        
        if widget_port and widget_port != '80' and widget_port != '443':
            return f"{protocol}://{widget_host}:{widget_port}"
        else:
            return f"{protocol}://{widget_host}"
